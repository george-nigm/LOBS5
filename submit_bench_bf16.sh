#!/bin/bash
###############################################################################
# Parallel LOB Benchmark for dandy-aardvark-138 (bf16 model)
# Submits 21 independent SLURM jobs, each computing one metric
###############################################################################

# Configuration
DATA_DIR="/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/lob_bench/output/dandy-aardvark-138"
SAVE_DIR="/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/lob_bench/output/dandy-aardvark-138/results"
STOCK="GOOG"
TIME_PERIOD="2023"
MODEL="s5-bf16"
...
echo "============================================"
echo "Submitting ${#METRICS[@]} parallel SLURM jobs for bf16 model"
echo "============================================"
...
for metric in "${METRICS[@]}"; do
    JOB_ID=$(sbatch --parsable \
        --job-name="bench_bf16_${metric}" \
        --output="logs_lobs5/bench_bf16_${metric}_%j.out" \
        --error="logs_lobs5/bench_bf16_${metric}_%j.err" \
        --nodes=1 \
        --ntasks-per-node=1 \
        --cpus-per-task=256 \
        --mem=512G \
