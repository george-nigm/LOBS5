...
def construct_custom_msg(
    ...
    Returns:
        custom_msg: The constructed message
        level_consumed: Boolean array indicating if the entire level was consumed (SIZE == avail)
    """
    ...
    )  # 0 = buy -> hit ask, 1 = sell -> hit bid

    # pick available volume on the level we will consume
    avail = jnp.where(
        DIRECTION_i == 0,
        best_ask_info[:, 1],  # ask vol for a buy order
        best_bid_info[:, 1],  # bid vol for a sell order
    ).astype(jnp.int32)
    ...
    print(f"SIZE: {SIZE} = jnp.minimum(jnp.int32({order_volume}), {avail})")

    SIZE = jnp.clip(SIZE, 1, 999_999)
    
    # Check if we consumed the entire level (SIZE == avail)
    level_consumed = (SIZE == avail)
    ...
    return custom_msg.astype(jnp.int32), level_consumed


def shift_sell_prices(
    msg: jnp.ndarray,
    tick_size: jnp.ndarray,
    PRICE_ABS_i: int,
    DIRECTION_i: int,
    EVENT_TYPE_i: int,
) -> jnp.ndarray:
    ...
    # print(f"msg[..., PRICE_ABS_i] + (1 - 2 * DIRECTION_i) * tick_size: {msg[..., PRICE_ABS_i].shape}, {(1 - 2 * DIRECTION_i)}, {tick_size.shape}")
    # print(f"msg[..., PRICE_ABS_i] + (1 - 2 * DIRECTION_i) * tick_size: {msg[..., PRICE_ABS_i]}, {(1 - 2 * DIRECTION_i)}, {tick_size}")

    # Reshape tick_size to match msg dimensions for broadcasting
    tick_size_reshaped = tick_size[:, None]  # (batch_size, 1)
    
    msg_shifted = msg_shifted.at[..., PRICE_ABS_i].set(
        jnp.where(mask, msg[..., PRICE_ABS_i] + (1 - 2 * DIRECTION_i) * tick_size_reshaped, msg[..., PRICE_ABS_i])
    )
    return msg_shifted
...
def run_historical_scenario(
    ...
    for batch_i in tqdm(sample_i):
        ...
        # shift_ticks = 0
        shift_ticks = [0] * batch_size
        
        level_consumed_flags: List[jnp.ndarray] = []  # Track when levels were fully consumed
        ...
        while original_idx < msg_seq_raw.shape[1] or insertion_iter < len(insertion_points):
            ...
            if insertion_iter < len(insertion_points) and steps_count == insertion_points[insertion_iter]:
                ...
                # print(f"[BATCH {batch_i}] Order volume: {order_volume}")

                sim_init, sim_state = inference.get_sims_vmap(current_book, last_msg[:, None, :])
                custom_msg, level_consumed = construct_custom_msg(
                    last_msg,
                    current_book,
                    sim_init,
                    sim_state,
                ...
                level_consumed_flags.append(level_consumed)

                # Only increment shift_ticks if we consumed the entire level
                # print(f"level_consumed: {level_consumed}")
                
                level_consumed_any = [int(x) for x in level_consumed]
                # print(f"level_consumed_any: {level_consumed_any}")

                # Element-wise addition instead of list concatenation
                shift_ticks = [shift_ticks[i] + level_consumed_any[i] for i in range(len(shift_ticks))]
                # print(f"Shift_ticks: {shift_ticks}")

                # Debug info
                # if jnp.any(level_consumed):
                #     print(f"[BATCH {batch_i}] Level consumed at insertion {insertion_iter}, shift_ticks now: {shift_ticks}")
            ...
            if original_idx < msg_seq_raw.shape[1]:
                ...
                # Historical messages don't consume levels, so add False flags
                level_consumed_flags.append(jnp.zeros(msg.shape[0], dtype=jnp.bool_))
                # print(f"[BATCH {batch_i}] 2 Level consumed flags: {level_consumed_flags}")
