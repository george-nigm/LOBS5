...
def _shard_for_pmap(x, num_devices):
    """Shard array along the leading axis for pmap. JIT-friendly version without prints."""
    if x is None:
        return None
...
def _unshard_from_pmap(x):
    """Unshard array from pmap output. JIT-friendly version without prints."""
    if x is None:
        return None
...
@functools.partial(jax.jit, static_argnames=('EVENT_TYPE_i', 'DIRECTION_i', 'tick_size', 'use_relative_volume', 'order_volume_ratio', 'order_volume'))
def insert_custom_end(
        m_seq_gen_doubled,
        b_seq_gen_doubled,
        msgs_decoded_doubled,
    ...
    # Use jnp.where instead of if/else for JIT compatibility
    best_ask = jax.vmap(sim_init.get_best_ask)(sim_states_init)
    best_bid = jax.vmap(sim_init.get_best_bid)(sim_states_init)
    PRICE_i = jnp.where(DIRECTION_i == 0, best_ask, best_bid)
    ...
    size_relative = jnp.floor(ratio * jnp.float32(avail)).astype(jnp.int32)
    # if there's some liquidity but rounding gave 0, take 1; if avail==0, keep 0
    size_relative = jnp.where((avail > 0) & (size_relative == 0), 1, size_relative)
    size_fixed = jnp.minimum(jnp.int32(order_volume), avail)
    SIZE_i = jnp.where(use_relative_volume, size_relative, size_fixed)
    ...
    for batch_i in tqdm(sample_i):
        ...
        for iteration in range(1,num_iterations+1):
            ...
            if iteration == 1:
                ...
                # Keep data on device until needed for saving
            ...
            # The encoder might contain nested structures. Optimized version without prints
            def process_encoder_recursive(obj, batch_size):
                if isinstance(obj, dict):
                    processed = {}
                    for key, value in obj.items():
                        processed[key] = process_encoder_recursive(value, batch_size)
                    return processed
            ...
            encoder_broadcast = process_encoder_recursive(encoder, batch_size)
            ...
            # Transfer to CPU and convert to numpy only once, right before saving
            # This minimizes device synchronization overhead
