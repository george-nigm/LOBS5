...
#SBATCH --output=logs/train_production_%j.out
#SBATCH --error=logs/train_production_%j.err
#SBATCH --partition=workq

set -e
echo "============================================================"
echo "ES-LOBS5 Production Training G5 (24h) + WandB"
echo "============================================================"
echo "Job ID: $SLURM_JOB_ID | Node: $SLURMD_NODENAME | Start: $(date)"
echo "============================================================"
echo "Config: n_perturbations=1024, n_steps=10, bg_msgs/step=100, task_size=500"
echo "============================================================"

mkdir -p logs
mkdir -p checkpoints/es_production
...
class Config:
    ...
    task_size: int = 500
    tick_size: int = 100
    checkpoint_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production'
    checkpoint_every: int = 10
    seed: int = 42
    output_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production'
...
run = wandb.init(
    project='ES-LOBS5',
    name='G5_steps10_bg100_task500',
    config=asdict(config),
    tags=['G5', 'production', '24h']
)
...
for epoch in range(config.n_epochs):
    ...
    if elapsed > max_training_time:
        print(f'Time limit reached ({elapsed/3600:.1f}h). Stopping.')
        break
