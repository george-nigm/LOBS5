#!/bin/bash
# =============================================================================
# RWKV v2 market impact experiments with cooling = 10 × insertions
# Grid: 10 configs × 3 volumes × 2 directions = 60 experiments on 7 GPUs
#
# Constraint: total_gen = (i + 10*i) * mb = 11*i*mb ≤ 500
#
# Usage:
#   chmod +x lob_impact/run_rwkv_c10x_v2.sh
#   ./lob_impact/run_rwkv_c10x_v2.sh
# =============================================================================
set -euo pipefail

PROJECT_DIR="/scratch/local/homes/80/georgenigm/LOBS5"
CONFIGS_DIR="${PROJECT_DIR}/lob_impact/configs_rwkv_c10x_v2"
LOGS_DIR="${PROJECT_DIR}/output/evalsequences/rwkv_goog2022/logs_c10x_v2"
...
SAVE_BASE="/home/myuser/data/evalsequences/rwkv_goog2022/c10x_v2"
N_COND=500
...
run_one() {
    ...
    local container_name="rwkv_v2_${job_name}"
...
for pair in "${GRID[@]}"; do
    ...
    for vol in "${VOLUMES[@]}"; do
        for dir in 0 1; do
            ...
            write_config "$cfg_host" "$i" "$c" "$vol" "$save_dir"

            job_name="i${i}_c${c}_mb${mb}_v${vol}_${dir_name}"
...
OUTPUT_BASE="${PROJECT_DIR}/output/evalsequences/rwkv_goog2022/c10x_v2"
