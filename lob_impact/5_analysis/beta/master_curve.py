...
def main():
    ...
    if not build_up:
        # one theoretical relaxation reference: propagator-shaped descent from the peak toward
        # the permanent level ≈ 2/3 of peak (fair pricing): ref(v) = 2/3 + 1/3·(v^0.5-(v-1)^0.5)
        from mid_trajectory import propagator_curve
        rt = ref_style('twothirds')
        px, prop = propagator_curve(1.0, 1.0, None, 0.5, float(vgrid[-1]))
        ax.plot(px, 2 / 3 + prop / 3, color=rt['color'], ls=rt['ls'], lw=2.0,
