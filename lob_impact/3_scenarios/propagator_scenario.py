...
def shift_prices(msg: jnp.ndarray, shift_amount: jnp.ndarray, direction: int) -> jnp.ndarray:
    """Shift prices of limit/execution historical messages to compensate for consumed liquidity.

    sign = (1 - 2*direction): direction=0 (buy, hitting ask) -> shift UP; direction=1 (sell) -> DOWN.
    msg: (batch, 14) raw historical messages; shift_amount: (batch,) = tick_size * shift_ticks.
    """
    sign = 1 - 2 * direction
    is_sell_relevant = (
        ((msg[:, EVENT_TYPE_i] == 1) | (msg[:, EVENT_TYPE_i] == 4)) & (msg[:, DIRECTION_i] == 0)
    )
    msg = msg.at[:, PRICE_ABS_i].set(
        jnp.where(is_sell_relevant, msg[:, PRICE_ABS_i] + sign * shift_amount, msg[:, PRICE_ABS_i])
    )
    is_buy_relevant = (
        ((msg[:, EVENT_TYPE_i] == 1) | (msg[:, EVENT_TYPE_i] == 4)) & (msg[:, DIRECTION_i] == 1)
    )
    msg = msg.at[:, PRICE_ABS_i].set(
...
def run_propagator_scenario(cfg: Dict[str, Any], save_folder: Path):
    ...
    # Sample indices — heuristic replay is DETERMINISTIC per window: a duplicate window is a
