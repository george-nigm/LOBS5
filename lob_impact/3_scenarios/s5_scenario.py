...
# ── Step 0: S5 (OLD context-scale) codebase (BEFORE any lob.*/s5.* import) ──
# No MAMBA3_LEGACY_NORM / TOKEN_MODE: S5 has no legacy-norm shim, and EXP_H2's default lob.encoding
# IS already 24-tok (matches the j2504167 ckpt). No sys.modules encoding shim needed.

# OLD S5 codebase: lob.*/s5.* must resolve here (original S5 ssm.py + 24-tok lob/encoding.py).
EXP = "/lus/lfs1aip2/projects/public/s5e/quant_team/quant/AlphaTrade/experiments/exp_H2-context-scale"
...
from lob_impact.core import inference_w_insertions_s5 as inference
...
def sample_aggressive_scenario(
    ...
    # S5: no tp_size / legacy-norm shim (those are Mamba3-only; the S5 args Namespace has no
    # mamba3_*/tp_size and EXP_H2/init_train builds S5 unconditionally — no ssm_type branch).
    ...
    # PARAMS-ONLY restore. The S5 ckpt's optimizer state (multi_transform regular/ssm, list-based) has a
    # different tree structure than the freshly-built one (dict-based) -> StandardRestore(state) rejects it
    # even with partial_restore. For inference we only need params. Restore the FULL on-disk 'state' using
    ...
    # Long-context conditioning (n_cond=4000) yields FEW windows on short days (EA ~20). Sample WITH
    # REPLACEMENT when n_samples exceeds the window count — each reused window generates a DISTINCT
    # sample (different RNG through the stochastic S5 generation).
    ...
    # Initialize hidden state template (S5 recipe).
    # Mirrors OLD sample_new (EXP_H2/lob/inference_no_errcorr.py:1216-1222). The S5 per-layer carry is
    # complex64 zeros (batch,1,ssm_size//2) — conj_sym halves the state. NO mamba3 head/rope/d_state args.
    init_hidden = model.initialize_carry(
        1,
        hidden_size=args.ssm_size_base // 2 ** int(args.conj_sym),
