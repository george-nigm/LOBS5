#!/usr/bin/env python
"""
Queue-Reactive (HLR) Aggressive Scenario: queue-reactive LOB generation (arrival intensities are functions of queue sizes;
Huang-Lehalle-Rosenbaum 2015) with aggressive order injection.
...
import queue_reactive as hawkes  # QR core, hawkes-compatible interface
...
def run_qr_scenario(cfg: Dict[str, Any], save_folder: Path,
    ...
    print(f"Loading queue-reactive params from {params_file}")
    params = hawkes.load_params(params_file)
    print(f"QR params: K={params['K']} n_bins={params['n_bins']} tick_size={params['tick_size']} "
          f"aes={params['aes']:.1f} p_inside={params['p_inside']:.3f}")
    print(f"  event_counts(MO,LO,CO)={onp.asarray(params['event_counts'])}")
    print(f"  deep_rate(LO,CO)={onp.asarray(params['deep_rate'])}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Queue-Reactive (HLR) Aggressive Scenario")
    parser.add_argument(
        '--config', '-c',
        type=str,
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Queue-Reactive (HLR) Aggressive Scenario Experiment")
    ...
    try:
        if cfg.get('per_day_params'):
            ...
            for day_idx, row in pd_df.iterrows():
                ...
                run_qr_scenario(cfg_d, save_folder, worker_id=worker_id, num_workers=num_workers)
        else:
            run_qr_scenario(cfg, save_folder, worker_id=worker_id, num_workers=num_workers)
