...
FLEET_NOTES = [
    ...
    (r'^(AMD|MSFT|AAPL)_noi_', 'no-insertion control: 64 samples, MB_OVERRIDE 100x m_b (4k pair resubmitted bsz 8)'),
    (r'^(AMD|MSFT|AAPL)_inv_', 'invisible-metaorder control: PER_DAY=1 N_PER_DAY=8, bsz 8'),
    (r'^ow_', 'OW (Obizhaeva-Wang exp kernel, tau=1000) generation: 52 samples/day, both shapes x dirs'),
    (r'^qrest_', 'QR (queue-reactive) parameter estimation from historic data_cond'),
    (r'^qr_', 'QR (queue-reactive) generation: 52 samples/day, both shapes x dirs'),
    (r'^s0[A-Z]?_|^s0_', 'Stage-0 noins scale-up to 2048 samples: SAMPLE_SLICE k/8 (CPU/500ctx) or k/16 (4k)'),
    (r'^est$|^est_', 'CST+Hawkes param estimation for one stock (EST_GRID override for AAPL)'),
    (r'^aggfig_', 'aggregation figure: raw spaghetti -> k-aligned mean + duration-independence'),
    (r'^dkf_', 'decay-kernel fit (flat/PL/exp by AIC) from master-curve cache'),
    (r'^fig5c', 'Fig 5 combined render (all-stock loop, sample-count table appended)'),
    (r'^b3l2|^b33', 'delta(k) estimator grids over the full pools (3 cross-sections)'),
    (r'^bsg', 'sigma-grid cube render (18 estimator x sigma cells per view)'),
    (r'^lbb_noins', 'LOB-Bench scoring of noins rollouts: 21 metrics x KS/L1/WS, all models per stock'),
    (r'^Y_amd', 'Y amplitude postprocess from cached 3x3 pools (AMD+MSFT)'),
    (r'^fb_lobs5', 'LobS5 legacy flow-balance (gamma_flow/H) from GOOG-2023 noins'),
    (r'^causal', 'placebo + pre-trend + flow-balance causal battery for one stock'),
    (r'^lobimp_master', 'gated master curves, both shapes, one stock'),
    (r'^mtd_', 'mid-trajectory cache dump for one stock'),
    (r'^p0_', 'placebo schedule run on free rollouts'),
    (r'^biasall', 'bias exhibit across all stocks'),
    (r'^lobimp_a5_beta', 'binned-delta headline pass (18 quantile bins, signed means)'),
    (r'^mresp', 'model R(m) response curves per stock'),
    (r'^lobimp_aggfig', 'paper aggregation figure job'),
