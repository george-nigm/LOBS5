...
_FIG = [Path('/app/pics_for_fair_beta_c10x_v2'),
        Path('/homes/80/georgenigm/LOBS5/pics_for_fair_beta_c10x_v2')]
...
metrics_all = ['Global', 'Per-config median', 'First-impact (k=1)',
               'Folder-weighted', 'Easy', 'Hard']
# Drop metrics that are all-NaN (e.g. Hard when max mb < 50)
metrics = [m for m in metrics_all if summary_df[m].notna().any()]
...
plot_metrics = [m for m in ['Global', 'Per-config median', 'First-impact (k=1)',
                'Folder-weighted', 'Easy', 'Hard'] if summary_df[m].notna().any()]
