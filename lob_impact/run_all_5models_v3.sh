#!/bin/bash
# =============================================================================
# All 5 scenarios — v3 grid (fixed cooling=5, wider mb range)
#
# Grid design:
#   - Constraint: i * (mb + 1) ≤ 500  (insertions fit within 500 messages)
#   - Fixed cooling: num_coolings = 5
#   - Max insertions per mb: i = floor(500 / (mb + 1))
#
# Grid (7 pairs × 3 volumes × 2 directions = 42 configs per scenario):
#   mb=5:   i=83  → insertion_phase=498, total=523
#   mb=10:  i=45  → insertion_phase=495, total=545
#   mb=20:  i=23  → insertion_phase=483, total=583
#   mb=25:  i=19  → insertion_phase=494, total=619
#   mb=50:  i=9   → insertion_phase=459, total=709
#   mb=75:  i=6   → insertion_phase=456, total=831
#   mb=100: i=4   → insertion_phase=404, total=904
#
# Scenarios (sequential): S5 → CGAN → CST → Historic → Heuristic
# Total: 5 × 42 = 210 experiments
#
# Usage:
#   chmod +x lob_impact/run_all_5models_v3.sh
#   ./lob_impact/run_all_5models_v3.sh              # full run
#   ./lob_impact/run_all_5models_v3.sh --dry-run    # generate configs only
# =============================================================================
set -euo pipefail
...
LOGS_BASE="${PROJECT_DIR}/output/evalsequences/logs_v3"
...
write_config_s5() {
    local file=$1 n_ins=$2 save_dir=$3 vol=$4
    cat > "$file" << EOF
n_gen_msgs: 50
...
run_one() {
    ...
    local container_name="v3_${scenario_type}_${job_name//%/pct}"
...
run_scenario() {
    ...
    local configs_dir="${PROJECT_DIR}/lob_impact/configs_${scenario_name}_v3"
    mkdir -p "$configs_dir"
    ...
    for pair in "${GRID[@]}"; do
        ...
        for vol in "${VOLUMES[@]}"; do
            for dir in 0 1; do
                ...
                save_dir="${SAVE_BASE}/${save_subdir}/v3/context_${N_COND}_${dir_name}/${folder_name}"
                workload=$total

                cfg_host="${configs_dir}/cfg_i${i}_c5_mb${mb}_v${vol}_${dir_name}.yaml"
                cfg_container="lob_impact/configs_${scenario_name}_v3/cfg_i${i}_c5_mb${mb}_v${vol}_${dir_name}.yaml"
                $write_fn "$cfg_host" "$i" "$save_dir" "$vol"
    ...
    if ! $DRY_RUN; then
        ...
        local OUTPUT_BASE="${PROJECT_DIR}/output/evalsequences/${save_subdir}/v3"
...
echo "================================================================="
echo "  All 5 scenarios — v3 grid (cooling=5, wide mb range)"
echo "  Started: $(date)"
