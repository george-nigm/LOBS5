...
# S5 (OLD exp_H2-context-scale codebase) has NO encoding_1tok; the 1tok fast-path is Mamba3-only and
# unused by the S5 scenario (S5 generates token-by-token). Guard so the module loads; the *_1tok
# functions stay defined but are never called on the 24-tok S5 path.
try:
    from lob.encoding_1tok import (
        local_to_global_jax, global_to_local_jax, N_FIELDS, N_SPECIAL_TOKENS,
    )
except ImportError:
    local_to_global_jax = global_to_local_jax = N_FIELDS = N_SPECIAL_TOKENS = None
import chex
...
def _generate_msg(
    ...
    # jax.debug.print('p_mid_after_regular {}', p_mid_after_regular)

    # price change in ticks
    p_change_regular = ((p_mid_after_regular - p_mid) // tick_size)#.astype(jnp.int32)

    # get new book state (after regular message, before any aggressive insertion)
...
def generate(
    ...
    if conditional:
        ...
        # N=chunk_size controls how many chunks the conditioning sequence is
        # split into for the roll-hidden scan (mirrors the OLD-fork behaviour).
        N = chunk_size
