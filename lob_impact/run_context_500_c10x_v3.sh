...
# v3: 3 checkpoints with 24-token encoding
#
# Constraint: total_gen = 11*i*mb ≤ 500
#
# Grid (10 pairs × 3 volumes × 2 directions = 60 runs per checkpoint):
...
#
# Checkpoints (v3, 24-token encoding):
#   j2514440 — BEST (150M params), step 135458
#   j2504227 — BIGGEST (360M params), step 22872
#   j2504167 — 4K context (50M params), step 100378
#
# Usage:
#   chmod +x lob_impact/run_context_500_c10x_v3.sh
#   ./lob_impact/run_context_500_c10x_v3.sh [checkpoint_key]
#
#   checkpoint_key: j2514440 | j2504227 | j2504167 | all (default: all)
# =============================================================================
set -eo pipefail

PROJECT_DIR="/scratch/local/homes/80/georgenigm/LOBS5"
CONFIGS_DIR="${PROJECT_DIR}/lob_impact/configs_context_500_c10x_v3"
LOGS_DIR="${PROJECT_DIR}/output/evalsequences/aggressive_scenario_v3/logs_c10x_v3"
...
declare -A CKPT_PATH CKPT_STEP
CKPT_PATH[j2514440]="/home/myuser/data/checkpoints/lobs5_v3/j2514440_bkotgtm5_2514440"
CKPT_STEP[j2514440]=135458

CKPT_PATH[j2504227]="/home/myuser/data/checkpoints/lobs5_v3/j2504227_c4sz78k1_2504227"
CKPT_STEP[j2504227]=22872

CKPT_PATH[j2504167]="/home/myuser/data/checkpoints/lobs5_v3/j2504167_y0c4j6l3_2504167"
CKPT_STEP[j2504167]=100378

ALL_CKPTS=(j2514440 j2504227 j2504167)

# ---- Parse which checkpoint(s) to run ----
CKPT_ARG="${1:-all}"
if [ "$CKPT_ARG" = "all" ]; then
    RUN_CKPTS=("${ALL_CKPTS[@]}")
else
    if [ -z "${CKPT_PATH[$CKPT_ARG]+x}" ]; then
        echo "ERROR: Unknown checkpoint key '$CKPT_ARG'. Use: j2514440 | j2504227 | j2504167 | all"
        exit 1
    fi
    RUN_CKPTS=("$CKPT_ARG")
fi
...
# Run each checkpoint
# =============================================================================
for ckpt_key in "${RUN_CKPTS[@]}"; do
    ckpt_path="${CKPT_PATH[$ckpt_key]}"
    ckpt_step="${CKPT_STEP[$ckpt_key]}"
    ckpt_logs="${LOGS_DIR}/${ckpt_key}"
    mkdir -p "$ckpt_logs"

    echo ""
    echo "============================================================"
    echo "  Checkpoint: ${ckpt_key} (step ${ckpt_step})"
    echo "  Path: ${ckpt_path}"
    echo "============================================================"
    ...
    for pair in "${GRID[@]}"; do
        ...
        for vol in "${VOLUMES[@]}"; do
            for dir in 0 1; do
                ...
                save_dir="${SAVE_BASE}/${ckpt_key}/context_${N_COND}_${dir_name}/${folder_name}"
                workload=$total

                cfg_host="${CONFIGS_DIR}/${ckpt_key}_cfg_i${i}_c${c}_mb${mb}_v${vol}_${dir_name}.yaml"
                cfg_container="lob_impact/configs_context_500_c10x_v3/${ckpt_key}_cfg_i${i}_c${c}_mb${mb}_v${vol}_${dir_name}.yaml"
                write_config "$cfg_host" "$i" "$c" "$save_dir" "$vol" "$ckpt_path" "$ckpt_step"

                job_name="v3_${ckpt_key}_${folder_name}_${dir_name}"
    ...
    # Snake pattern: 1,2,3,4,5,6,7, 7,6,5,4,3,2,1, 1,2,...
    ...
    # ---- Print assignment ----
    echo ""
    echo "  === GPU Assignment (GPUs 1-7, GPU 0 excluded) ==="
    ...
    echo "  === Launching ${ckpt_key} on 7 GPUs... $(date) ==="

    for gpu in "${AVAILABLE_GPUS[@]}"; do
        if [ -s "${TMPDIR_JOBS}/gpu_${gpu}" ]; then
            run_gpu_from_file "$gpu" "$ckpt_logs" "${TMPDIR_JOBS}/gpu_${gpu}" &
        fi
    done
...
for ckpt_key in "${RUN_CKPTS[@]}"; do
    for context_dir in "${OUTPUT_BASE}/${ckpt_key}"/context_${N_COND}_*; do
        ...
        for folder_dir in "$context_dir"/i*; do
            ...
            if [ -n "$exp_dir" ]; then
                echo "  Flattening: ${ckpt_key}/${folder_dir##*/}"
